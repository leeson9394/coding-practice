    ...
    def longestStrChain(self, w):
        ...
        for word in words:
            if len(word) <= max_chain: # skip word if it cannot be greater than max_chain
                continue
            max_candidate = self.find_longest_chain(word, words, 0, [ 0 ])
            max_chain = max(max_candidate, max_chain)
        return max_chain

    def find_longest_chain(self, word, words, current_chain, max_chain):
        ...
        for i in range(len(word)):
            ...
            self.find_longest_chain(new_word, words, current_chain, max_chain)
        return max_chain[0]
